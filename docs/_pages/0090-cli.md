...
[**VSecM Sentinel**][sentinel]’s Command Line Interface (_CLI_).
...
You can use the `-a` (_append_) argument to register more than one secret
to a workload.
...
To transform the value to _YAML_, or _JSON_, `-s` has to be a **valid** _JSON_.
...
`"true"` and provide the _root key_ manually.
...
> provide the _root key_ to **VSecM Safe** whenever the pod is evicted, or
...
```bash
docker run --rm vsecm-keygen:0.22.4
...
To use these patterns, simply prefix the `-v` flag with `gen:` (_or
`VSECM_SENTINEL_SECRET_GENERATION_PREFIX` if you are using the environment
to override it_) as follows:
