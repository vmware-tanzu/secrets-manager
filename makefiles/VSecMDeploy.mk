...
deploy-spire:
	@if [ "${DEPLOY_SPIRE}" = "true" ]; then \
		...
		kubectl wait --timeout=60s --for=condition=Available statefulset -n spire-system spire-server; \
...
post-deploy:
	...
	kubectl wait --timeout=60s --for=condition=Available statefulset -n vsecm-system vsecm-safe
