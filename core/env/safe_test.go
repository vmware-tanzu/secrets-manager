...
func TestManualRootKeyUpdatesKubernetesSecrets(t *testing.T) {
	tests := []struct {
		name    string
		setup   func() error
		cleanup func() error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.setup != nil {
				if err := tt.setup(); err != nil {
					t.Errorf("ManualRootKeyUpdatesKubernetesSecret() = failed to setup, with error: %+v", err)
				}
			}
			defer func() {
				if tt.cleanup != nil {
					if err := tt.cleanup(); err != nil {
						t.Errorf("ManualRootKeyUpdatesKubernetesSecret() = failed to cleanup, with error: %+v", err)
					}
				}
			}()
			if got := ManualRootKeyUpdatesKubernetesSecret(); got != tt.want {
				t.Errorf("ManualRootKeyUpdatesKubernetesSecret() = %v, want %v", got, tt.want)
			}
		})
	}
}
