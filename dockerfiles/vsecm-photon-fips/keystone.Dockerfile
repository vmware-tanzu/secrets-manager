...
RUN CGO_ENABLED=0 GOEXPERIMENT=boringcrypto GOOS=linux go build -mod vendor -a -o vsecm-keystone \
    ./app/keystone/cmd/main.go

# generate clean, final image for end users
FROM photon:5.0

ENV APP_VERSION="0.24.2"
...
COPY --from=builder /build/vsecm-keystone /keystone
